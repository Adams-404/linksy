...
1. **Wireless Concurrent Wi-Fi Hotspot (`linksy on --wifi`)**: On dual-interface-capable Wi-Fi cards, Linksy automatically clones your upstream Wi-Fi frequency and channel into an independent virtual Access Point (`ap0`) with zero connection drops, automatic DHCP, and real-time device management. Includes **Intelligent Regional & Hardware NO-IR Auto-Recovery**: if your upstream Wi-Fi operates on a channel restricted from mobile phone scanning (like 5 GHz Channels 36–48 in Nigeria) or your Wi-Fi adapter's regulatory database marks a channel with `NO-IR` (passive scan only / AP mode forbidden), Linksy automatically switches your connection to the 2.4 GHz band so your phone can detect and connect to the hotspot instantly!
